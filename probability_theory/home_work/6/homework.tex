...
\]
Now we'll find the CDF of $Y$ and then differentiate to get the PDF:
\[
...
\]
For the last integral, we will use substitution $u=\sin x$, so $du=\cos x\,dx$:
\[
...
\]
Because of the symmetry of $f_X$, we have $f_X(\sqrt y)=f_X(-\sqrt y)=\frac12 e^{-\sqrt y}$. Therefore:
\[
...
Let's assume $\sigma_X>0$ and $\sigma_Y>0$ (otherwise the correlation isn't defined). Lets denote the centered variables as \(\widetilde X=X-\mathbb E[X]\) and \(\widetilde Y=Y-\mathbb E[Y]\).
...
Lets consider the quadratic form for any \(t\in\mathbb R\):
\[
...
Now let's find the marginal PDFs. For $|x|\le 1$, we will denote $a=\sqrt{1-x^2}$:
\[
...
\]
To evaluate this integral, we will use the antiderivative formula:
\[
\int \sqrt{x^2+y^2}\,dy
...
\]
where $\sqrt{x^2+a^2}=\sqrt{x^2+1-x^2}=1$. Therefore, for $|x|\le 1$:
\[
...
\]
Notice that the joint PDF doesn't depend on $\theta$ at all. Let's verify this leads to independence. The normalization gives:
\[
...
\]
so $R$ and $\Theta$ are independent. Since the distribution is spherically symmetric, it is rotationally invariant, which means the angle is uniformly distributed and independent of the distance from the origin.
